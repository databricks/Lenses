...
object Lens {
  ...
  implicit class SeqLikeLens[U, A, Coll[A] <: collection.SeqOps[A, Coll, Coll[A]]](val lens: Lens[U, Coll[A]]) extends AnyVal {
  ...
  implicit class SetLikeLens[U, A, Coll[A] <: collection.SetOps[A, Coll, Coll[A]]](val lens: Lens[U, Coll[A]]) extends AnyVal {
    type CBF = collection.generic.CanBuildFrom[Coll[A], A, Coll[A]]
